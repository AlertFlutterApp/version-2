import 'package:flutter/material.dart';
import 'package:final_app/bloc/bloc_provider.dart';
import 'package:final_app/pages/tasks/bloc/task_bloc.dart';
import 'package:final_app/pages/labels/label_db.dart';
import 'package:final_app/pages/projects/project_db.dart';
import 'package:final_app/pages/projects/project.dart';
import 'package:final_app/pages/about/about_us.dart';
import 'package:final_app/pages/home/home_bloc.dart';
import 'package:final_app/pages/labels/label_bloc.dart';
import 'package:final_app/pages/labels/label_widget.dart';
import 'package:final_app/pages/projects/project_bloc.dart';
import 'package:final_app/pages/projects/project_widget.dart';
import 'package:final_app/utils/keys.dart';

class SideDrawer extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    HomeBloc homeBloc = BlocProvider.of(context);
    return Drawer(
      child: ListView(
        padding: EdgeInsets.all(0.0),
        children: <Widget>[
          UserAccountsDrawerHeader(
            accountName: Text("Alert Flutter App"),
            accountEmail: Text("َAlertFlutterApp@gmail.com"),
            otherAccountsPictures: <Widget>[
              IconButton(
                  icon: Icon(
                    Icons.info,
                    color: Colors.white,
                    size: 36.0,
                  ),
                  onPressed: () {

                    //showAboutDialog(context: context,applicationName: "flutter app" );
                    Navigator.push(
                      context,
                      MaterialPageRoute<bool>(
                          builder: (context) => AboutUsScreen()),
                    );
                  })
            ],
            currentAccountPicture: CircleAvatar(
              backgroundColor: Theme.of(context).accentColor,
             backgroundImage: AssetImage("assets/profile_pic.jpg"),
            ),
          ),
          ListTile(
              leading: Icon(Icons.inbox),
              title: Text(
                "صندوق", //Inbox
                key: ValueKey(SideDrawerKeys.INBOX),
              ),
              onTap: () {
                var project = Project.getInbox();
                homeBloc.applyFilter(
                    project.name, Filter.byProject(project.id));
                Navigator.pop(context);
              }),
          ListTile(
              onTap: () {
                homeBloc.applyFilter("امروز", Filter.byToday());
                Navigator.pop(context);
              },
              leading: Icon(Icons.calendar_today),
              title: Text(
                "امروز",
                key: ValueKey(SideDrawerKeys.TODAY),
              )),
          ListTile(
            onTap: () {
              homeBloc.applyFilter("هفت روز آینده", Filter.byNextWeek()); //Next 7 Days
              Navigator.pop(context);
            },
            leading: Icon(Icons.calendar_today),
            title: Text(
              "هفت روز آینده",
              key: ValueKey(SideDrawerKeys.NEXT_7_DAYS),
            ),
          ),
          BlocProvider(
            bloc: ProjectBloc(ProjectDB.get()),
            child: ProjectPage(),
          ),
          BlocProvider(
            bloc: LabelBloc(LabelDB.get()),
            child: LabelPage(),
          )
        ],
      ),
    );
  }
}
